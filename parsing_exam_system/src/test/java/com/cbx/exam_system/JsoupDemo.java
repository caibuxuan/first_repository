...
public class JsoupDemo {
	...
	public static void main(String[] args) {
		try {
			...
			// 指定文件名及路径
			File file = new File("F:\\jsoup\\word\\test.txt");
			if (!file.exists()) {
				file.createNewFile();
			}
			// 写入本地
			PrintWriter pw = new PrintWriter("F:\\jsoup\\word\\test.txt", "UTF-8");
			...
			for (Element element : imgs) {
				...
				// 写入本地
				os = new FileOutputStream(new File("F:\\jsoup\\img", imageName));
